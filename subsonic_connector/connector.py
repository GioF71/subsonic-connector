...
class Connector:
    ...
    def getCoverArtForGenre(
        ...
        songs: dict = self.__connect().getSongsByGenre(
            ...
            maxSongs)
        if not songs:
            raise Exception("No songs for genre [{}]".format(genre))
    ...
    def getTopSongs(
            self,
            artist: str,
            count: int = 50) -> Response[TopSongs]:
    ...
    def getArtistInfo(
            ...
            aid,
            count=20,
            includeNotPresent=False) -> Response[ArtistInfo]:
    ...
    def getCoverByArtistId(self, artist_id: str) -> ArtistCover:
        ...
        if not artist_response and not artist_response.getObj():
            return None
    ...
    def getStarred(self) -> Response[Starred]:
        data: dict = self.__connect().getStarred2()
    ...
    def buildSongUrlBySong(self, song: Song, format: str = None, max_bitrate: int = None) -> str:
        ...
        if format:
            url_dict["format"] = format
        if max_bitrate:
            url_dict["maxBitRate"] = max_bitrate
